                    ...
                    >
                        ...
                        <div className="flex-1 space-y-4">
                            <div className="bg-white/5 rounded-xl p-5 space-y-4 border border-white/5">
                                ...
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-400 text-sm font-medium">Profit Split ({getSelectedAccount()?.type === 'direct_funded' ? '30%' : '20%'} Fee)</span>
                                    <span className="text-red-400 text-xl font-bold tracking-tight">-${(parseFloat(amount) * (getSelectedAccount()?.type === 'direct_funded' ? 0.30 : 0.20)).toFixed(2)}</span>
                                </div>
                                <div className="h-px bg-white/10 my-2" />
                                <div className="flex justify-between items-center pt-1">
                                    <span className="text-gray-300 text-sm font-medium uppercase tracking-wider">Total You Receive ({getSelectedAccount()?.type === 'direct_funded' ? '70%' : '80%'})</span>
                                    <span className="text-blue-400 text-2xl font-bold tracking-tight">${(parseFloat(amount) * (getSelectedAccount()?.type === 'direct_funded' ? 0.70 : 0.80)).toFixed(2)}</span>
                                </div>
                            </div>
