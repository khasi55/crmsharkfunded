...
async function sendMigrationEmails() {
    console.log('🔄 Starting MT5 Account Migration Email Broadcast...');
    
    const csvPath = '/Users/viswanathreddy/Desktop/Desktop - VISWANATH’s MacBook Pro/Sharkfunded/SharkfundedCRM/swapped_account_users.csv';
    
    if (!fs.existsSync(csvPath)) {
        console.error('❌ Could not find swapped_account_users.csv');
        return;
    }
    ...
    console.log(`📦 Found ${lines.length} users to email.\n`);

    for (let i = 0; i < lines.length; i++) {
        const parts = lines[i].split(',');
        const email = parts[parts.length - 1].trim().replace(/"/g, '');
        
        if (!email || !email.includes('@')) {
            console.warn(`⚠️ Skipping invalid email on line ${i+2}: ${lines[i]}`);
            continue;
        }

        try {
            console.log(`[${i + 1}/${lines.length}] Sending to ${email}...`);
        ...
        // Small delay to prevent SMTP rate limits
