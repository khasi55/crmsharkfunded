...
async function main() {
    console.log('Searching for email containing "gojiyanayan9"');

    // 1. Find user in auth.users or public.profiles
    const { data: profiles, error: profileErr } = await supabase
        .from('profiles')
        .select('*')
        .ilike('email', '%gojiyanayan9%');
    ...
    if (!profiles || profiles.length === 0) {
        console.error('Profile not found for this email substring.');
        return;
    }

    const userId = profiles[0].id;
    console.log(`Found User ID: ${userId} (${profiles[0].full_name || 'No Name'}), Email: ${profiles[0].email}`);
    ...
    if (orders && orders.length > 0) {
        orders.forEach((o, i) => {
            ...
            console.log(`Gateway: ${o.payment_gateway}`);
            console.log(`Data: ${JSON.stringify(o.payment_details)}`);
        });
