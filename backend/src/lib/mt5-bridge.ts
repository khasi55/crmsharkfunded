...
export async function adjustMT5Balance(login: number, amount: number, comment: string = 'Payout Deduction') {
    const mt5ApiUrl = process.env.MT5_BRIDGE_URL || process.env.MT5_API_URL || 'https://bridge.sharkfunded.co';

    const response = await fetch(`${mt5ApiUrl}/deposit`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-API-Key': process.env.MT5_API_KEY || ''
    ...
    if (!response.ok) {
        const errText = await response.text();
        throw new Error(`Bridge balance adjustment error: ${errText}`);
    }
...
export async function changeMT5Leverage(login: number, leverage: number) {
    ...
    console.log(`🔌 [Bridge] Changing Leverage for ${login} to ${leverage}`);

    const response = await fetch(`${mt5ApiUrl}/change-leverage`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-API-Key': process.env.MT5_API_KEY || ''
    ...
    if (!response.ok) {
        const errText = await response.text();
        throw new Error(`Bridge leverage change error: ${errText}`);
    }
